    ...
    */
	...
	var NativeFormatter = /** @class */ (function () {
		...
		NativeFormatter.prototype.formatRange = function (start, end, context) {
			...
			var separator =
				extendedSettings.separator || context.defaultSeparator || '';
			if (insertion) {
				return (
        ...
        */
		...
		EventApi.prototype.formatRange = function (formatInput) {
			...
			var formatter = createFormatter(
				formatInput,
				this._context.options.defaultRangeSeparator,
			);
			if (this._def.hasEnd) {
				return dateEnv.formatRange(
	...
	function reduceDateProfile(
		...
		switch (action.type) {
			...
			case 'CHANGE_DATE':
				...
				break;
			case 'PREV':
				var dp0 = dateProfileGenerator.buildPrev(
					currentDateProfile,
					currentDate,
				);
				if (dp0.isValid) {
				...
				break;
			case 'NEXT':
				var dp1 = dateProfileGenerator.buildNext(
					currentDateProfile,
					currentDate,
				);
				if (dp1.isValid) {
    ...
    */
		...
		CalendarDataManager.prototype.updateData = function () {
			...
			if (oldCalendarOptions && oldCalendarOptions !== newCalendarOptions) {
				if (oldCalendarOptions.timeZone !== newCalendarOptions.timeZone) {
					data.eventSources = reduceEventSourcesNewTimeZone(
						data.eventSources,
						state.dateProfile,
						data,
					);
					data.eventStore = rezoneEventStoreDates(
						data.eventStore,
						oldData.dateEnv,
						data.dateEnv,
					);
	...
	function buildDateProfileGenerator(props) {
		var DateProfileGeneratorClass =
			props.viewSpec.optionDefaults.dateProfileGeneratorClass ||
			DateProfileGenerator;
				...
				if (_this.isDragging) {
					...
					if (validMutation) {
						...
						if (receivingContext === initialContext_1) {
							...
							var eventDropArg = __assign(__assign({}, transformed), {
								el: ev.subjectEl,
										...
										return createElement(
											'a',
											{
												...
												className: ['fc-daygrid-week-number']
													.concat(classNames)
													.join(' '),
												'data-navlink': options.navLinks
													? buildNavLinkData(date, 'week')
													: null,
											},
											innerContent,
										);
						...
						createElement(
							...
							createElement(
								'a',
								{
									...
									'data-navlink': _this.context.options.navLinks
										? buildNavLinkData(props.date)
										: null,
								},
								innerContent,
							),
	...
	function renderInnerContent$2(innerProps) {
		return createElement(
			...
			createElement('div', {
				...
				style: {
					backgroundColor: innerProps.backgroundColor || innerProps.borderColor,
				},
			}),
			innerProps.timeText &&
				createElement(
				...
				if (!segIsHidden[seg.eventRange.instance.instanceId]) {
					...
					if (seg.firstCol === seg.lastCol && seg.isStart && seg.isEnd) {
						...
						segMarginTops[seg.eventRange.instance.instanceId] =
							placement.top -
							currentBottom + // from previous seg bottom
							currentExtraSpace;
						currentExtraSpace = 0;
					} else {
    ...
    */
	...
	var TableRow = /** @class */ (function (_super) {
			...
			return createElement(
				...
				props.cells.map(function (cell, col) {
					...
					return createElement(TableCell, {
						...
						innerElRef: _this.cellInnerElRefs.createRef(
							cell.key,
						...
						fgContentElRef: _this.cellContentElRefs.createRef(cell.key),
		...
		TableRow.prototype.renderFgSegs = function (
			...
			var _a = this.state,
				cellInnerPositions = _a.cellInnerPositions,
				cellContentPositions = _a.cellContentPositions;
				...
				for (var _i = 0, segs_1 = segs; _i < segs_1.length; _i++) {
					...
					if (isAbsolute) {
						...
						if (context.isRtl) {
							right = seg.isStart
								? 0
								: cellContentPositions.rights[seg.firstCol] -
								  cellInnerPositions.rights[seg.firstCol];
							left =
								(seg.isEnd
									? cellContentPositions.lefts[seg.lastCol]
									: cellInnerPositions.lefts[seg.lastCol]) -
								cellContentPositions.lefts[seg.firstCol];
						} else {
							left = seg.isStart
								? 0
								: cellInnerPositions.lefts[seg.firstCol] -
								  cellContentPositions.lefts[seg.firstCol];
							right =
								cellContentPositions.rights[seg.firstCol] -
								(seg.isEnd
									? cellContentPositions.rights[seg.lastCol]
									: cellInnerPositions.rights[seg.lastCol]);
						}
					} else {
		...
		TableRow.prototype.renderFillSegs = function (segs, fillType) {
			...
			if (cellInnerPositions) {
				for (var _i = 0, segs_2 = segs; _i < segs_2.length; _i++) {
					...
					var eventRange = seg.eventRange;
					var key = eventRange.instance
						? eventRange.instance.instanceId
						: eventRange.def.defId;
					nodes.push(
						createElement(
							'div',
							{ className: 'fc-daygrid-bg-harness', style: leftRightCss },
							fillType === 'bg-event'
								? createElement(
										BgEvent,
										__assign(
		...
		TableRow.prototype.updateSizing = function (isExternalSizingChange) {
			var _a = this,
				props = _a.props,
				cellInnerElRefs = _a.cellInnerElRefs,
				cellContentElRefs = _a.cellContentElRefs;
			if (props.clientWidth !== null) {
				// positioning ready?
				if (isExternalSizingChange) {
					var cellInnerEls = props.cells.map(function (cell) {
						return cellInnerElRefs.currentMap[cell.key];
					});
					var cellContentEls = props.cells.map(function (cell) {
						return cellContentElRefs.currentMap[cell.key];
					});
			...
			if (allDayContent) {
				...
				sections.push({
					...
					outerContent: createElement(
						...
						createElement('td', {
							className:
								'fc-timegrid-divider fc-divider ' +
					...
					return createElement(
						...
						createElement(
							...
							createElement(
								...
								{
									className:
										'fc-list-day-frame ' + theme.getClass('tableCellShaded'),
									ref: innerElRef,
								},
								innerContent,
							),
	...
	function renderInnerContent$4(props) {
		return createElement(
			...
			props.text &&
				createElement(
					'a',
					{ className: 'fc-list-day-text', 'data-navlink': props.navLinkData },
					props.text,
				),
			props.sideText &&
				createElement(
